 ...
 * @version 1
 */
public class PrevencionColisionesLinked {
        ...
        for (int i = 0; i < arregloDeAbejas.length; ++i) {
            for (int j = i + 1; j < arregloDeAbejas.length; ++j) {
                if (distancia(arregloDeAbejas[i], arregloDeAbejas[j]) <= 100) {
                    ...
                    if (!abejasConRiesgoDeColision.contains(arregloDeAbejas[i])) {
                        abejasConRiesgoDeColision.add(arregloDeAbejas[i]);
                    }
                    if (!abejasConRiesgoDeColision.contains(arregloDeAbejas[j])) {
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        Point3D[] arregloDeAbejas = leerArchivo(10000);
